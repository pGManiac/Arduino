      ...
      try{
        ...
        if(rest2 == 0)
        {
          //try{
            do {
                  drv.delay_ms(20);
                  drv.setRegister(&PORTA, 0xF0);
                  drv.delay_ms(20);
                  start = (drv.getRegister(&PINA) & 0x0F) << 4;
                  std::cerr << std::endl;
                  std::cerr << uint8_t_to_binary_re(start) << std::endl;
                  //uint8_t_to_binary(start);
                }while(start != 0xF0);
        ...
        //Lesen
        drv.delay_ms(50);
        message = drv.getRegister(&PINA) & 0x0F;
        ...
        if(switchToCheck == false)
        {
            ...
            }else if(rest == 1)
            {
                ...
                outputU8 = (outputTop << 4) | outputBottom;
                /*if(outputU8 != 0x03){
        ...
        }else if(switchToCheck == true)
        {
          ...
          std::cerr << "ReadChecksum: " << uint8_t_to_binary_re(sentChecksum) << std::endl;
          std::cerr << "BufferChecksum: " << uint8_t_to_binary_re(checksumBuffer) << std::endl;
          std::cerr << "ReceivedBlock: " << std::flush;
          //std::cout << "ReceivedBlock: " << std::flush;
          if(sentChecksum == checksumBuffer)
          {
            for(int i = 0; i < buffer.size(); i++)
            {
              std::cout << buffer[i] << std::flush;
            }
          }else if(sentChecksum != checksumBuffer)
          {
        ...
        {
            std::cerr << " Progamm beendet!" << std::endl;
            run = 0;
        }
...
int createChecksum(std::string inputStr)
{
    ...
    return intChecksum;
        ...
        drv.setRegister(&PORTA, 0);
        cout << "Schreibe " << zahl << endl; //int, uint8_t würde fehlerhaft wiedergegeben
        drv.setRegister(&PORTA, (intToU8 << 4));


        cout << ((int)drv.getRegister(&PINA) & 0x0F) << endl;
        drv.delay_ms(300);*/
